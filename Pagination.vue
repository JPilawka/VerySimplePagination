<template>
  <div class="paginationContainer">
    <div v-if="totalPages() > 0"
          class="btn-group pagination-group "
          role="group"
          aria-label="Pagination btns">
      <button
              type="button"
              v-if="showPreviousLink()"
              class="btn btn-secondary btn-primary"
              @click="updatePage(currentPage - 1)"> PREV </button>
      <button
              v-if="currentPage != 0"
              type="button"
              class="btn btn-secondary btn-primary"
              @click="updatePage(0)"> FIRST</button>
      <!--Display first 5 pages -->
      <span
            v-for="button in shownButtons"
            class="btn-group"
            role="group"
            aria-label="Pagination btns">
        <button
                :class="['btn', 'btn-secondary',{ 'btn-success':(isCurrentPage(button)), 'btn-primary':(!isCurrentPage(button))}]"
      ...
      <button
              ...
              type="button"
              class="btn btn-secondary btn-primary"
              @click="updatePage(totalPages()-1)"> LAST</button>
      <button
              type="button"
              v-if="showNextLink()"
              class="btn btn-secondary btn-primary"
